...
class eventficlink(Cog):
    ...
    async def on_ready(self):
        if not self.bot.ready:
            self.bot.cogs_ready.ready_up('eventficlink')
    ...
    async def on_message(self, message):
        ...
        if ctx.command is None and igncheck not in message.content and\
                "https://archiveofourown.org/works/" in message.content:
...
# embed formatting for AO3 work embed
            try:
                ...
                if cha == "on":
                    embedVar.add_field(name="Characters:", value=characters,
                                       inline=False)
                else:
                    pass
